/* ═══════════════════════════════════════════════════════
   ...
   Archivo separado para evitar problemas de parser HTML
═══════════════════════════════════════════════════════ */
(function() {
...
var TC_PROMPTS = {
  leo: 'Eres Leo, el asistente de bienvenida de TodoCamping (empresa de alquiler de campers en Madrid). FECHA HOY: ' + TC_HOY + '. MISION: detectar la intencion y derivar con la herramienta derivar_agente. TONO: cercano, breve, tuteas. Deriva a "informativo" para info de campers/precios/disponibilidad. Deriva a "reservas" para reservar/cancelar/consultar reservas. Responde tu mismo solo para saludos, contacto o intenciones ambiguas (haz una sola pregunta). Usa SIEMPRE la herramienta derivar_agente para derivar.',
  cami: 'Eres Cami, asistente informativa de campers de TodoCamping. FECHA HOY: ' + TC_HOY + '. MONEDA: Pesos argentinos ($). Nunca uses euros. Usa SIEMPRE la herramienta consultar_campers antes de dar datos. Recomienda 1-3 opciones. Cuando el usuario quiera reservar, usa pasar_a_reservas.',
  reservas: 'Eres Remi, agente de reservas de TodoCamping. FECHA HOY: ' + TC_HOY + '. MONEDA: Pesos argentinos ($). ORDEN: 1) pedir modelo+fechas, 2) verificar_disponibilidad, 3) pedir datos personales, 4) mostrar resumen, 5) crear_reserva tras confirmacion. Si piden info de campers usa pasar_a_cami.',
};

var TC_TOOLS = {
  leo: [{
    name: 'derivar_agente',
    description: 'Deriva al agente correcto segun la intencion del usuario.',
  ...
  cami: [
    { name:'consultar_campers', description:'Consulta el catalogo de campers disponibles.', input_schema:{ type:'object', properties:{ estado:{type:'string'}, disponible:{type:'boolean'}, tipo:{type:'string'}, capacidad_min:{type:'integer'}, precio_max:{type:'number'}, tiene_ac:{type:'boolean'}, tiene_bano:{type:'boolean'} } } },
    { name:'pasar_a_reservas', description:'Transfiere al agente de reservas cuando el usuario quiere reservar.', input_schema:{ type:'object', properties:{ mensaje_usuario:{type:'string'}, contexto:{ type:'object', properties:{ resumen:{type:'string'}, modelo:{type:'string'}, fechas:{type:'string'}, pax:{type:'integer'} }, required:['resumen'] } }, required:['mensaje_usuario','contexto'] } },
  ],
  reservas: [
    { name:'verificar_disponibilidad', description:'Verifica si un camper esta disponible para fechas dadas.', input_schema:{ type:'object', properties:{ modelo:{type:'string'}, fecha_inicio:{type:'string'}, fecha_fin:{type:'string'} }, required:['modelo','fecha_inicio','fecha_fin'] } },
    { name:'crear_reserva', description:'Crea una reserva. Solo tras confirmacion explicita.', input_schema:{ type:'object', properties:{ camper_modelo:{type:'string'}, cliente_nombre:{type:'string'}, cliente_documento:{type:'string'}, cliente_telefono:{type:'string'}, cliente_email:{type:'string'}, fecha_inicio:{type:'string'}, fecha_fin:{type:'string'}, num_personas:{type:'integer'} }, required:['camper_modelo','cliente_nombre','cliente_documento','cliente_telefono','cliente_email','fecha_inicio','fecha_fin'] } },
    { name:'consultar_reserva', description:'Busca una reserva por email o ID.', input_schema:{ type:'object', properties:{ email:{type:'string'}, id_reserva:{type:'string'} } } },
    { name:'listar_reservas_activas', description:'Lista todas las reservas activas.', input_schema:{ type:'object', properties:{}, required:[] } },
    { name:'cancelar_reserva', description:'Cancela una reserva. Solo tras confirmacion.', input_schema:{ type:'object', properties:{ id_reserva:{type:'string'}, motivo:{type:'string'} }, required:['id_reserva'] } },
    { name:'buscar_disponibilidad_alternativa', description:'Busca campers alternativos disponibles en las mismas fechas.', input_schema:{ type:'object', properties:{ fecha_inicio:{type:'string'}, fecha_fin:{type:'string'}, num_personas:{type:'integer'} }, required:['fecha_inicio','fecha_fin'] } },
    { name:'pasar_a_cami', description:'Pasa al agente informativo Cami para info de modelos.', input_schema:{ type:'object', properties:{ mensaje_usuario:{type:'string'}, contexto:{ type:'object', properties:{ resumen:{type:'string'} }, required:['resumen'] } }, required:['mensaje_usuario','contexto'] } },
  ],
};
...
function tcValidarFechas(fi, ff) {
  ...
  if (a < hoy) {
    var sug = new Date(a); sug.setFullYear(hoy.getFullYear()); if(sug<hoy) sug.setFullYear(hoy.getFullYear()+1);
    return { ok:false, error:'La fecha de inicio ya paso. Querias decir ' + sug.getFullYear() + '-' + String(sug.getMonth()+1).padStart(2,'0') + '-' + String(sug.getDate()).padStart(2,'0') + '?' };
  }
  if (b <= a) return { ok:false, error:'La fecha de fin debe ser posterior a la de inicio.' };
  return { ok:true };
}

function tcFindCamperMock(q) {
...
async function tcFindCamper(q) {
  ...
  if (tcSb) {
    try {
      var r1 = await tcSb.from('campers').select('*').ilike('key', q).limit(1);
      if (r1.data && r1.data[0]) return r1.data[0];
      var r2 = await tcSb.from('campers').select('*').ilike('modelo', '%'+q+'%').limit(1);
      if (r2.data && r2.data[0]) return r2.data[0];
    } catch(e) {}
  }
  return tcFindCamperMock(q);
}
...
async function tcExecConsultarCampers(f) {
  ...
  if (tcSb) {
    try {
      var q = tcSb.from('campers').select('*');
      if (f.estado)       q = q.eq('estado', f.estado);
      if (f.disponible!=null) q = q.eq('disponible', f.disponible);
      if (f.tipo)         q = q.eq('tipo', f.tipo);
      if (f.capacidad_min)q = q.gte('capacidad', f.capacidad_min);
      if (f.precio_max)   q = q.lte('precio_diario', f.precio_max);
      if (f.tiene_ac)     q = q.eq('ac', true);
      if (f.tiene_bano)   q = q.eq('bano', true);
      var res = await q.order('precio_diario', { ascending: true });
      if (!res.error && res.data) data = res.data;
    } catch(e) {}
  } else {
    if (f.estado)        data = data.filter(function(c){ return c.estado===f.estado; });
    if (f.disponible!=null) data = data.filter(function(c){ return c.disponible===f.disponible; });
    if (f.tipo)          data = data.filter(function(c){ return c.tipo===f.tipo; });
    if (f.capacidad_min) data = data.filter(function(c){ return c.capacidad>=f.capacidad_min; });
    if (f.precio_max)    data = data.filter(function(c){ return c.precio_diario<=f.precio_max; });
    if (f.tiene_ac)      data = data.filter(function(c){ return c.ac; });
    if (f.tiene_bano)    data = data.filter(function(c){ return c.bano; });
    data.sort(function(a,b){ return a.precio_diario-b.precio_diario; });
  }
...
async function tcExecVerificarDisp(args) {
  ...
  if (camper.estado !== 'Activo') return { disponible:false, motivo:'El camper esta en estado '+camper.estado+'.' };
  if (!camper.disponible) return { disponible:false, motivo:'El camper no esta disponible.' };
  var ocupadas = 0;
  if (tcSb) {
    try {
      var r = await tcSb.from('reservas').select('*',{count:'exact',head:true}).eq('camper_key',camper.key).in('estado_reserva',['Confirmada','Pendiente']).lte('fecha_inicio',args.fecha_fin).gte('fecha_fin',args.fecha_inicio);
      ocupadas = r.count || 0;
    } catch(e) {}
  } else {
    ocupadas = TC_MOCK.reservas.filter(function(r){ return r.camper_key===camper.key && ['Confirmada','Pendiente'].includes(r.estado_reserva) && r.fecha_inicio<=args.fecha_fin && r.fecha_fin>=args.fecha_inicio; }).length;
  }
  var disp = (camper.unidades||1) - ocupadas;
  return { disponible:disp>0, modelo:camper.modelo, key:camper.key, precio_por_noche:camper.precio_diario, moneda:'ARS ($)', unidades_disponibles:Math.max(0,disp) };
}

async function tcExecCrearReserva(args) {
  var fv = tcValidarFechas(args.fecha_inicio, args.fecha_fin);
  if (!fv.ok) return { error:fv.error };
  if (!tcValidarEmail(args.cliente_email)) return { error:'Email invalido: "'+args.cliente_email+'".' };
  if (args.cliente_documento && !tcValidarDoc(args.cliente_documento)) return { error:'Documento invalido.' };
  if (args.cliente_telefono && !tcValidarTel(args.cliente_telefono)) return { error:'Telefono invalido. Debe tener entre 7 y 15 digitos.' };
...
async function tcExecConsultarReserva(args) {
  if (tcSb) {
    try {
      var q = tcSb.from('reservas').select('*');
      if (args.id_reserva) q = q.eq('reserva_ref', args.id_reserva);
      else if (args.email)  q = q.ilike('cliente_email', args.email);
      var r = await q.limit(10);
      return r.data && r.data.length ? { reservas:r.data } : { resultado:'No se encontraron reservas.' };
    } catch(e) {}
  }
  var found = TC_MOCK.reservas;
  if (args.id_reserva) found = found.filter(function(r){ return r.reserva_ref===args.id_reserva||r.id===args.id_reserva; });
  else if (args.email) found = found.filter(function(r){ return r.cliente_email&&r.cliente_email.toLowerCase()===args.email.toLowerCase(); });
  return found.length ? { reservas:found } : { resultado:'No se encontraron reservas.' };
}

async function tcExecListarActivas() {
  if (tcSb) {
    try {
      var r = await tcSb.from('reservas').select('*').in('estado_reserva',['Confirmada','Pendiente']).order('fecha_inicio',{ascending:true});
      return { reservas:r.data||[], total:(r.data||[]).length };
    } catch(e) {}
  }
...
async function tcExecCancelar(args) {
  if (tcSb) {
    try {
      var u = { estado_reserva:'Cancelada' }; if(args.motivo) u.notas=args.motivo;
      var r = await tcSb.from('reservas').update(u).eq('reserva_ref',args.id_reserva);
    ...
    if (result.stop_reason === 'tool_use') {
      ...
      for (var i=0; i<result.content.length; i++) {
        ...
        if (block.name==='derivar_agente') {
          handoff = { agente:block.input.agente_destino, contexto:block.input.contexto, mensajeVisible:block.input.mensaje_usuario }; output={ok:true};
        } else if (block.name==='pasar_a_reservas') {
          handoff = { agente:'reservas', contexto:block.input.contexto, mensajeVisible:block.input.mensaje_usuario }; output={ok:true};
        } else if (block.name==='pasar_a_cami') {
          handoff = { agente:'cami', contexto:block.input.contexto, mensajeVisible:block.input.mensaje_usuario }; output={ok:true};
      ...
      if (handoff) { history.push({ role:'assistant', content:result.content }); return { text:handoff.mensajeVisible, handoff:handoff }; }
      messages.push({ role:'user', content:toolResults });
      history.push({ role:'user', content:toolResults });
      continue;
    }

    var fallbackText = result.content&&result.content.find&&result.content.find(function(b){return b.type==='text';});
    return { text:fallbackText?fallbackText.text:'Respuesta inesperada.', handoff:null };
  }
  return { text:'Lo siento, tardo demasiado. Podrias repetir tu consulta?', handoff:null };
}
...
async function tcPerformHandoff(handoff) {
  ...
  var brief = '[El usuario fue derivado. Contexto: ' + ctx.resumen + '.';
  if (ctx.modelo) brief += ' Modelo: ' + ctx.modelo + '.';
  if (ctx.fechas) brief += ' Fechas: ' + ctx.fechas + '.';
  if (ctx.pax)    brief += ' Personas: ' + ctx.pax + '.';
  brief += ' Saluda brevemente y empieza a ayudar directamente.]';
  tcSetTyping(true);
  try {
...
async function tcIniciar() {
  tcInitSupabase();
  tcUpdateTheme(TC_AGENTS.leo);
  tcSetTyping(true);
  tcSetDisabled(true);
  try {
    var result=await tcCallClaude(TC_PROMPTS.leo, TC_TOOLS.leo, [{role:'user',content:'(El usuario acaba de abrir el chat. Saludalo brevemente y preguntale en que puedes ayudarle.)'}]);
    var text=result.content.filter(function(b){return b.type==='text';}).map(function(b){return b.text;}).join('');
    tcSetTyping(false);
